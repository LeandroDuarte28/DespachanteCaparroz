...
class CaparrozDespachanteSupabaseUser extends BaseAuthUser {
  CaparrozDespachanteSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> caparrozDespachanteSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CaparrozDespachanteSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
